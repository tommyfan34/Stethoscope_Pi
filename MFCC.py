...
def MFCC(signal, sample_rate):
    ...
    # Transform the frequncy to Mel frequncy
    nfilt = 40
    low_freq_mel = 0
...
if __name__ == "__main__":
    ...
    for i, yi in enumerate(path):
        ...
        mfcc = MFCC(wavdata, samplerate)
        plt.figure(i, figsize=(10, 4))
        plt.title("MFCC")
        plt.xlabel("time(s)")
        plt.ylabel("MFCC vector")
        # plt.xticks(numpy.arange(audio_clip[0], audio_clip[1], 0.5))
        plt.imshow(numpy.flipud(mfcc.T), cmap=plt.cm.RdBu, aspect='auto', extent=[0,mfcc.shape[0]*0.01,0,mfcc.shape[1]])
        plt.colorbar()
    plt.show()